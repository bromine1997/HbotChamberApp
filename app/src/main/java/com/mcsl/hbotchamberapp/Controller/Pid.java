...
public class Pid {
    ...
    public void setPID(double p, double i, double d){
        P=p;D=d;
        //Note: the I term has additional calculations, so we need to use its
     ...
     * Increasing the filter strength will reduce P and D oscillations, but force larger I
     ...
     */
    private double constrain(double value, double min, double max){             // 값을 특정 범위로 제한
     ...
     */
    private boolean bounded(double value, double min, double max){                  // 값이 범위 내에 있는지 확인
     ...
     */
    private void checkSigns(){                                          /// PID 매개변수가 올바른 부호를 가지고 있는지 확인
